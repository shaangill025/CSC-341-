...
/**
 * This is the Update Service Controller for the UpdateServiceMenu.fxml.
 * 
 * @author sfyock
 *
 */
public class UpdateServiceController {
	...
	void initialize() {
		...
		servFeeField.setTextFormatter(new TextFormatter<>(c -> {
			...
			if (object == null || parsePosition.getIndex() < c.getControlNewText().length()) {
				return null;
			} else if (c.getControlNewText().length() > 6) {
				return null;
			} else {
				return c;
			}
	...
	void setErrorLabel(Label l, String str) {
		l.setText(l.getText() + "To update your Service you are missing:" + str);
	}
